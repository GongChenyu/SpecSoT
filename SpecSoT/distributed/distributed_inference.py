...
try:
    # 当作为模块导入时使用相对导入
    from .comm_manager import create_zmq_comm_manager
    from ..kv_cache import initialize_past_key_values
    from ..modeling_qwen3_kv import Qwen3ForCausalLM
except ImportError:
    # 当直接运行时使用绝对导入
    from SpecSoT.distributed.comm_manager import create_zmq_comm_manager
    from SpecSoT.kv_cache import initialize_past_key_values
    from SpecSoT.modeling_qwen3_kv import Qwen3ForCausalLM
...
class DistributedInferenceEngine:
    ...
    def _setup_logger(self):
        """设置日志"""
        logger = logging.getLogger(f"Distributed-Rank-{self.rank}")
