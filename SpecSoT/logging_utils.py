...
使用方式：
    >>> from SpecSoT.logging_utils import get_unified_logger
    >>> logger = get_unified_logger(rank=0, log_dir="logs")
...
def get_unified_logger(
    ...
    Args:
        ...
        name_suffix: logger名称后缀，用于区分不同模块
    ...
    # 确定logger名称
    if rank >= 0:
        logger_name = f"SpecSoT-Rank{rank}{name_suffix}"
    else:
        logger_name = f"SpecSoT{name_suffix}"
    ...
    if log_dir is None:
        ...
        # logging_utils.py 在 SpecSoT/ 目录下，所以向上一级是项目根目录
